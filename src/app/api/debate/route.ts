...
export async function POST(req: Request) {
  ...
  try {
    ...
    const estimatedCost = estimateCostTokens(requestType);
    ...
    if (!isFreeTrialRequest) {
      const newBalance = profile.token_balance_cents - 1;
    ...
    await notifyAdmin('Critical API Failure', `Debate Route Error: ${message}`);
