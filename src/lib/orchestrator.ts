...
export class DebateOrchestrator {
    ...
    async executeStep(
        ...
        for (const providerName of candidates) {
            ...
            try {
                ...
                console.warn(`Step failed on agent ${agent.name} (${providerName}), trying next...`, error.message);
                lastError = error;
    ...
    private async callOpenAI(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        ...
        } catch (error: any) { return { content: `[OpenAI Error: ${error.message}]`, status: 'failed' as const }; }
    }
    ...
    private async callDeepSeek(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        try {
            ...
            const data = await response.json();
            return { content: data.choices[0]?.message?.content || '', status: 'success' as const };
        } catch (error: any) { return { content: `[DeepSeek Error: ${error.message}]`, status: 'failed' as const }; }
    }
    ...
    private async callAnthropic(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        ...
        } catch (error: any) { return { content: `[Anthropic Error: ${error.message}]`, status: 'failed' as const }; }
    }
    ...
    private async callGoogle(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        ...
        } catch (error: any) { return { content: `[Google Error: ${error.message}]`, status: 'failed' as const }; }
    }
    ...
    private async callMistral(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        try {
            ...
            return { content: `[Mistral Error: ${error.message}]`, status: 'failed' as const };
        }
    }
    ...
    private async callCohere(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        try {
            ...
            return { content: `[Cohere Error: ${error.message}]`, status: 'failed' as const };
        }
    }
    ...
    private async callReplicateText(agent: AIProvider, systemPrompt: string, userPrompt: string) {
        try {
            ...
            return { content: `[Replicate Error: ${error.message}]`, status: 'failed' as const };
        }
    }

    // Helpers
    private getFallbackPlan(providers: any[]): TaskPlan['sequence'] {
    ...
    async saveTurn(debateId: string, agentName: string, providerId: string, content: string) {
        await this.supabase.from('debate_turns').insert({
            debate_id: debateId,
            ai_provider_id: providerId,
            ai_name_snapshot: agentName, // This controls the display name
            content: content
        });
    }
