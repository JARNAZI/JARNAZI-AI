...
async function runVideoComposeFromPending(admin: AdminClient, userId: string, pending: PendingRow) {
  ...
  if (!composerUrl || !composerSecret) {
    // mark job failed
    await (admin.from('video_jobs') as any).update({ status: 'failed', error: 'Missing CLOUD_RUN_COMPOSER_URL/SECRET' }).eq('id', jobId);
  ...
  if (!dispatch.ok) {
    ...
    await (admin.from('video_jobs') as any).update({ status: 'failed', error: `Composer dispatch failed: ${dispatch.status} ${t}` }).eq('id', jobId);
    return { ok: false, error: 'Composer dispatch failed' };
  }

  // Mark pending as done (job created)
  await (admin.from('pending_requests') as any).update({ status: 'done', last_error: `job:${jobId}` }).eq('id', pending.id);
...
export async function processPendingForUser(userId: string) {
  ...
  if (Number.isFinite(exp) && exp <= Date.now()) {
    await (admin.from('pending_requests') as any).update({ status: 'expired' }).eq('id', pending.id);
