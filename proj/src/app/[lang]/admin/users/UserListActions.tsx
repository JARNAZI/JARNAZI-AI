...
export default function UserListActions({ userId, userEmail }: { userId: string, userEmail: string }) {
    ...
    const handleDelete = async () => {
        const confirmText = prompt(`Type "DELETE" to confirm deleting ${userEmail} forever.`);
    ...
    return (
        <div className="flex justify-end gap-2">
            <Link
                ...
                className="px-3 py-1 bg-white/10 hover:bg-white/20 rounded text-sm transition-colors text-white flex items-center gap-2"
            >
