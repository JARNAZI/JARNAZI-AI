  ...
  try {
    ...
    if (amount === null || !isValidPurchaseAmount(amount)) {
      return NextResponse.json(
        { error: 'Invalid amount. Minimum is $14 and amount must be a whole number (e.g. 14, 20, 50).' },
        { status: 400 }
      );
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${appUrl}/${body?.lang || 'en'}/debate?purchase=success`,
      cancel_url: `${appUrl}/${body?.lang || 'en'}/buy-tokens?purchase=cancel`,
    });
